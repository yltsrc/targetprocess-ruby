...
module Assignable
  ...
  ARR_VAR = %w( tags skills )
  FLOAT_VAR = %w( weeklyavaolablehours currentavailablehours availablefuturehours timeremain timespent efforttodo effortcompleted effort initialestimate)
  INT_VAR = %w( currentallocation availablefutureallocation role_id id entitystate_id priority_id team_id teamiteration_id iteration_id release_id project_id lastcommenteduser_id owner_id entitytype_id numericpriority userstory_id severity_id build_id )
  DATE_VAR = %w( deletedate availablefrom startdate enddate createdate modifydate lastcommentdate )
  ALL_VAR = [:id, :name , :description, :startdate, :enddate, :createdate, :modifydate, :lastcommentdate, :tags, :numericpriority, :effort, :effortcompleted, :efforttodo, :timespent, :timeremain, :entitytype, 
             ...
             :priority_id, :priority_name, :entitystate, :entitystate_id, :entitystate_name, :customfields]
  ...
  module InstanceMethods
    ...
    def ==(obj)
      (self.class::ALL_VAR+self.class.missings).each { |var| return false unless self.send(var) == obj.send(var) }
      true
    end
    ...
    def normalize_values  
      klass = self.class
      (klass::ALL_VAR+klass.missings).each do |var|
